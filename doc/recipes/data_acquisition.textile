...
Ruport's DataSet loading, but it sure is sweet :)
...
So for example, have a look at the DataSet below.
...
Once loaded into a DataSet, <code>my_loaded_data["bar"]</code>
and <code>my_loaded_data[1]</code> become equivalent.
...
h3. Default Values for Empty Cells

Empty cells will automatically be converted to nil. However, if you'd like 
them to be converted to some other value, you can pass this via the
<code>:default</code> option.
...
even though your results are converted into DataSets, you will see a much smaller
...
Below is a snippet of row by row processing for DataSet creation from CSV.  The
block yields two objects, the first is the DataSet which will represent your
...
generating a DataRow for them.  The snippet below will create a DataSet which
...
The general use case will be to generate a DataSet from a SQL query's results.
This can then be used in all the ways a regular Ruport DataSet can be used. This
is quite easy to do.

Assuming you already have a default source set, the example below shows how to
run a trivial query and get back a DataSet.
...
You can also treat queries as enumerable objects, which will yield DataRows by
default.
...
ways.  Think of a cached query object as a sort of 'dataset generator'.
...
Please note that although Generators can be handy, they are quite slow.  This is
because they are implemented using continuations.  So if performance is a
concern, you should probably use some other way of iterating through your data,
and you should probably also read the next section.

For the adventurous, Generator is much faster in Ruby 1.9.  For the slightly
less adventurous, you can probably hotswap a solution to RubyQuiz 66
(http://rubyquiz.com/quiz66.html) if you really want to get wild and crazy.  
For the not so adventurous, forget you read this paragraph :)
...
you should probably never, ever try to build a DataSet with greater 
than 5 or 10k records. This performance issue may be overcome in the future if I
see sufficient need for it in the community, but for now, DataSets are really not
...
The following example shows how you can build a DataSet using DBI::Rows as the
...
before you load data into your DataSet, which is much better than converting all
...
allows you to convert a model or a find on a model into a DataSet.  From there,
...
h3. Easy DataSet Conversion

If you've got a relatively small model and you need most of the data from it to
do some reporting, it couldn't get easier to convert it into a DataSet:
...
a DataSet would be costly (and most likely painful).
...
just the same as Ruport's DataSets and Table outputs.  This means you are free
