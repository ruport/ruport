...
Data::Table or some other format that *does* display field names by default 
...
<pre><code>
  ...
  table = Ruport::Format.table_object :data => my_data, :plugin => :csv
...
Data::Table which contains the field "email" :
...
a Data::Table or an ActiveRecord object.

h4. Data::Collection Shortcut

<tt>Data::Collection#as</tt> is a simple wrapper around Format.table which saves a few
keystrokes and makes things look nicer (I think)

The following code will output a Data::Table as a CSV:

<pre><code>
  table = Data::Table.new :column_names => %w[a b], :data => [[1,2],[3,4]]
  puts table.as(:csv)
</code></pre>
...
<pre><code>
  puts table.as(:csv) { |engine| engine.show_field_names = false }
</code></pre>
...
(optionally) and then have it converted to a Data::Table.  This method takes a
